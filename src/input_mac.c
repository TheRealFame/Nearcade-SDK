...
{
    (void)config;
    LOG_WARN("macOS stub backend (no gamepad injection API)");
    return NEARCADE_OK;
}

void input_shutdown(void)
{
    LOG_DEBUG("input_shutdown (macOS stub)");
}

int input_alloc_slot(uint8_t slot, uint16_t vendor, uint16_t product,
                     uint16_t version, const char *name)
{
    LOG_DEBUG("input_alloc_slot: slot=%d (macOS stub, no-op)", slot);
...
{
    LOG_DEBUG("input_free_slot: slot=%d (macOS stub, no-op)", slot);
    (void)slot;
    return NEARCADE_OK;
}
...
{
    LOG_TRACE("input_submit_gamepad: slot=%d (macOS stub, dropping)", pkt ? pkt->slot : -1);
    (void)pkt;
    return NEARCADE_OK;
}
...
{
    LOG_TRACE("input_submit_kbm: viewer=%s event=%s (macOS stub, dropping)",
...
{
    LOG_DEBUG("input_flush_slot: slot=%d (macOS stub, no-op)", slot);
    (void)slot;
    return NEARCADE_OK;
}
